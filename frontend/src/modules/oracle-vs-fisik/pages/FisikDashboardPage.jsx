...
import { SCAN_ACTIVITY, PROGRESS_GEDUNG_A, PROGRESS_GEDUNG_B, PATTERN_SUMMARY } from "../data";
...
function ProgressCell(row) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 120 }}>
      ...
      <span className="mono" style={{ fontSize: 11, color: "var(--text-secondary)" }}>{row.progres}%</span>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="stat-grid">
        <StatCard icon={ScanLine} label="Qty Ter-scan Hari Ini" value={totalScan.toLocaleString("id-ID")} tone="accent" />
        <StatCard icon={Users} label="Operator Aktif" value={activePic} tone="cyan" />
        <StatCard icon={Warehouse} label="Gedung Berjalan" value="2" sub="Gedung A & Gedung B" tone="ok" />
        <StatCard icon={Gauge} label="Progres Rata-rata" value="78%" tone="warn" />
      </div>
