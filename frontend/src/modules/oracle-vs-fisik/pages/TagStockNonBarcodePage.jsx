...
function TagStockNonBarcodePage() {
  const columns = [
    { key: "no", label: "No.", render: (_, i) => i + 1 },
    {
      key: "lot",
      label: "Lot",
      render: (r) => <span className="cell-code">{r.lot}</span>,
    },
    ...
    { key: "jml_aktual", label: "Jml Aktual", align: "right" },
    {
      ...
      render: (r) =>
        r.qty === r.jml_aktual ? (
          <StatusBadge tone="ok">SESUAI</StatusBadge>
        ) : (
          <StatusBadge tone="danger">
            SELISIH {r.jml_aktual - r.qty}
          </StatusBadge>
        ),
    },
  ];
  ...
  return (
    ...
    >
      <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 0 }}>
        Dipakai untuk item yang dihitung manual (belum ter-cover barcode), input
        lewat upload Excel jumlah rak & aktual per lot.
      </p>
